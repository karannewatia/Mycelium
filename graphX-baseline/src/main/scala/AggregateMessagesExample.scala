...
object AggregateMessagesExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val graph: Graph[Double, Int] =
      GraphGenerators.logNormalGraph(sc, numVertices = 10000000, mu=10).mapVertices( (id, _) => id.toDouble % 2)
    // Compute the number of older followers and their total age
    val olderFollowers: VertexRDD[(Int, Double)] = graph.aggregateMessages[(Int, Double)](
      triplet => { // Map Function
        if (triplet.srcAttr == triplet.dstAttr && triplet.srcAttr == 1) {
    ...
    val avgAgeOfOlderFollowers: VertexRDD[Double] =
      olderFollowers.mapValues( (id, value) =>
        value match { case (count, totalAge) => count } )
    // Display the results
